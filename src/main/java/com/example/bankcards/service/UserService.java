...
public class UserService {
    ...
    public UserDTO registerUser(UserRegisterDTO dto) {
        ...
        if (userRepository.findByEmailOrPhoneNumber(dto.getEmail(), dto.getPhoneNumber()).isPresent()) {
    ...
    public UserDTO getUser(Long id) {
        ...
        if (!isAdmin(auth) && !userEntity.getEmail().equals(auth.getName())) {
            log.warn("Отказано в доступе к пользователю ID {} для пользователя: {}", id, auth.getName());
