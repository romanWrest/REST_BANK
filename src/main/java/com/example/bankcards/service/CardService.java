...
public class CardService {
    ...
    public CardResponseBlockDTO requestBlock(Long cardId) {
        ...
        CardEntity cardEntity = cardRepository.findById(cardId)
                .orElseThrow(() -> {
                    ...
                    return new ResourceNotFoundException("Карта с id " + cardId + " не найдена");
                });
    ...
    public List<CardDTO> getAllCards(Pageable pageable) {
        ...
        if (pageable == null) {
            log.error("Pageable равен null для getAllCards");
            throw new IllegalArgumentException("Pageable не может быть null");
        }
        ...
        log.info("Успешно получено {} карт", result.size());
        return result;
    }
